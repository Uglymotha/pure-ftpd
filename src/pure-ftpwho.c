...
int main(int argc, char *argv[])
{
    ...
    int fodder;
#ifdef USE_CAPABILITIES
    cap_flag_value_t cap_dac_override;
    cap_get_flag(cap_get_proc(), CAP_DAC_OVERRIDE, CAP_EFFECTIVE, &cap_dac_override);
#endif

#ifndef NON_ROOT_FTP
    if (geteuid() != (uid_t) 0
#ifdef USE_CAPABILITIES
        && cap_dac_override != CAP_SET
#endif
       ) {
...
# endif
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (fstat(mmap_fd, &st) != 0 || !S_ISREG(st.st_mode) ||
            ...
            (st.st_uid != (uid_t) 0 && cap_dac_override != CAP_SET)
#else
            st.st_uid != (uid_t) 0
#endif
            ) {
