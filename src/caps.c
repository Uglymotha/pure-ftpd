...
int get_startup_caps(void)
{
    ...
    if (caps != NULL && i > 0 && i < CAP_STARTUP_SZ) {
        i = 0;
        perror("Dropping all capabilities, as not all required are available.");
        perror("Continuing, but pure-ftpd will not work as expected.");
        apply_caps(cap_none, 0, cap_none, 0, NULL, CAP_CLEAR);
    }
    cap_free(caps);
    return (i > 0 && i >= CAP_STARTUP_SZ);
}
